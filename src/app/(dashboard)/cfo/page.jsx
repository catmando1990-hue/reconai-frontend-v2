...
function normalizeDashboard(raw) {
  ...
  function wrapMetric(val, fallbackSentiment = 'neutral') {
    ...
    if (val !== null && val !== undefined) {
      return { value: val, state: METRIC_STATE.READY, sentiment: fallbackSentiment };
    }
  ...
  return {
    ...
    metrics: {
      ...
      totalExpenses: wrapMetric(m.total_expenses ?? m.totalExpenses, 'negative'),
...
const quickLinks = [
  { id: 'pnl', label: 'P&L Statement', icon: FileBarChart, href: '/reports/pnl' },
  { id: 'balance', label: 'Balance Sheet', icon: LineChart, href: '/reports/balance' },
  { id: 'cashflow', label: 'Cash Flow', icon: Activity, href: '/reports/cashflow' },
...
function MetricWidget({ label, metric, format = 'currency' }) {
  ...
  return (
    <div className={`metric-widget ${sentimentClass} ${!isReady ? 'unavailable' : ''}`}>
      <span className="metric-label">{label}</span>
      {isReady ? (
        <span className="metric-value">{formatValue(metric.value, format)}</span>
      ) : (
        ...
        {snapshot.highlights.map((item, index) => (
          <div key={index} className={`highlight ${item.sentiment}`}>
            {item.sentiment === 'positive' ? <TrendingUp size={14} /> :
             item.sentiment === 'warning' ? <AlertCircle size={14} /> :
...
function ReportPurpose() {
  ...
  return (
    <div className="report-purpose">
      ...
      <p className="purpose-description">This executive overview is designed for:</p>
      <ul className="purpose-list">
        {purposes.map((p, i) => (
          <li key={i}><p.icon size={14} /><span>{p.label}</span></li>
        ))}
      </ul>
    </div>
  );
...
function DataSourcePanel({ source }) {
  const lastSyncTime = new Date(source.lastSync).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
      ...
      <div className="source-info">
        ...
        <div className="source-institutions">
          {source.institutions.map((inst, i) => (
            <span key={i} className="institution-tag">{inst}</span>
          ))}
        </div>
      </div>
    </div>
...
function SignalCard({ signal, onAction }) {
  ...
  return (
    <div className={`signal-card ${signal.severity} ${signal.status}`}>
      <div className="signal-icon"><Icon size={14} /></div>
      <div className="signal-content">
        <span className="signal-title">{signal.title}</span>
        <p className="signal-description">{signal.description}</p>
        {signal.status === 'pending' && (
          <div className="signal-actions">
            <button className="signal-btn primary" onClick={() => onAction(signal.id, 'review')}>
              <Eye size={10} /> Review
            </button>
          </div>
        )}
...
function ComplianceRow({ item }) {
  ...
  return (
    <div className="compliance-row">
      ...
      <span className={`compliance-status ${config.className}`}>{item.status}</span>
    </div>
  );
}
...
export default function CFOOverview() {
  ...
  const fetchOverview = useCallback(async () => {
    ...
    try {
      ...
      setSignals(rawSignals.map(s => ({
        ...
        timestamp: s.timestamp ?? (s.created_at ? formatRelativeTime(s.created_at) : 'recently'),
        status: s.status ?? 'pending',
      })));

      // Populate compliance items from the dashboard response (if present)
      const rawCompliance = raw.compliance ?? raw.compliance_items ?? raw.complianceItems ?? [];
      if (rawCompliance.length > 0) {
        setComplianceItems(rawCompliance.map(c => ({
          id: c.id,
          name: c.name ?? c.title,
  ...
  const handleSignalAction = (id) => {
    setSignals(prev => prev.map(s => s.id === id ? { ...s, status: 'reviewed' } : s));
  };
    ...
    <div className="cfo-overview" data-lifecycle={lifecycle}>
      ...
      <header className="overview-header">
        <div className="header-left">
          ...
          <p className="header-subtitle">Executive snapshot for {metrics.period.value}</p>
        </div>
        <div className="header-right">
          ...
          <button className="header-btn" onClick={handleRefresh} disabled={isRefreshing} title="Refresh">
            <RefreshCw size={16} className={isRefreshing ? 'spinning' : ''} />
          </button>
          <div className="export-dropdown">
            <button className="header-btn primary" onClick={() => setShowExportMenu(!showExportMenu)} title="Export">
              <Download size={16} />
            </button>
            {showExportMenu && (
              <div className="export-menu">
                {exportOptions.map(opt => (
                  <button key={opt.id} className="export-option" onClick={() => handleExport(opt.id)}>
        ...
        <main className="primary-column">
          ...
          <div className="insights-row">
            <section className="compliance-section">
              <div className="section-header">
                <h2><Shield size={14} /> Compliance</h2>
              </div>
              <div className="compliance-list">
                {complianceItems.map(item => <ComplianceRow key={item.id} item={item} />)}
              </div>
            </section>

            <section className="signals-section">
              <div className="section-header">
                <h2><Zap size={14} /> AI Signals {pendingSignals > 0 && <span className="signal-count">{pendingSignals}</span>}</h2>
                <button className="section-action"><Filter size={12} /></button>
              </div>
              <div className="signals-list">
                {signals.filter(s => s.status !== 'dismissed').map(signal => (
        ...
        <aside className="secondary-column">
          <section className="metrics-summary">
            ...
            <div className="metrics-list">
              <MetricWidget label="Total Revenue" metric={metrics.totalRevenue} format="currency" />
              <MetricWidget label="Total Expenses" metric={metrics.totalExpenses} format="currency" />
              <MetricWidget label="Net Position" metric={metrics.netPosition} format="currency" />
              <MetricWidget label="Period" metric={metrics.period} format="text" />
              <MetricWidget label="Transactions" metric={metrics.transactionCount} format="number" />
            </div>
          </section>
      ...
      <footer className="overview-footer">
        <span className="last-updated"><Clock size={11} /> Updated: {lastUpdated.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
        <span className="data-source-footer"><Building2 size={11} /> {dataSource.connectedAccounts} accounts synced</span>
      </footer>
    </div>
  );
}
