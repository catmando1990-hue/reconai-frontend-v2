...
function generateTaxDocuments(taxYear, taxSummary, accounts) {
  ...
  if (savingsAccounts.length > 0 && interestTotal > 0) {
    documents.push({
      ...
      generatedData: {
        ...
        details: savingsAccounts.map((acc) => ({
          ...
          account: `****${acc.mask}`,
          interest:
            acc.interestEarned || interestTotal / savingsAccounts.length,
        })),
      },
    });
  } else {
    documents.push({
      ...
      generatedData:
        savingsAccounts.length > 0 ? { totalAmount: 0, itemCount: 0 } : null,
    });
  }
  ...
  if (hasBusinessIncome) {
    documents.push({
      ...
      generatedData: {
        ...
        payers: [
          ...new Set(transactionData.income.freelance.map((t) => t.source)),
        ],
  ...
  documents.push({
    ...
    autoGenerate: false,
    status:
      taxSummary.deductions.mortgageInterest > 0
        ? DOC_STATUS.READY
        : DOC_STATUS.NOT_APPLICABLE,
    sources: ["lender"],
  ...
  documents.push({
    ...
    autoGenerate: false,
    status:
      taxSummary.deductions.studentLoanInterest > 0
        ? DOC_STATUS.READY
        : DOC_STATUS.NOT_APPLICABLE,
    sources: ["loan_servicer"],
    lastUpdated:
      taxSummary.deductions.studentLoanInterest > 0 ? new Date() : null,
    generatedData:
      taxSummary.deductions.studentLoanInterest > 0
        ? {
  ...
  if (hasBusinessIncome) {
    ...
    documents.push({
      ...
      generatedData: {
        ...
        percentage:
          (
            (transactionData.business.homeOffice.officeSquareFeet /
              transactionData.business.homeOffice.totalSquareFeet) *
            100
          ).toFixed(1) + "%",
        breakdown: {
          mortgage:
            transactionData.business.homeOffice.annualMortgageInterest *
            (transactionData.business.homeOffice.officeSquareFeet /
              transactionData.business.homeOffice.totalSquareFeet),
          utilities:
            transactionData.business.homeOffice.annualUtilities *
            (transactionData.business.homeOffice.officeSquareFeet /
              transactionData.business.homeOffice.totalSquareFeet),
          insurance:
            transactionData.business.homeOffice.annualInsurance *
            (transactionData.business.homeOffice.officeSquareFeet /
              transactionData.business.homeOffice.totalSquareFeet),
          repairs:
            transactionData.business.homeOffice.annualRepairs *
            (transactionData.business.homeOffice.officeSquareFeet /
              transactionData.business.homeOffice.totalSquareFeet),
        },
      },
    ...
    // Mileage Log
    const totalMiles = transactionData.business.mileage.reduce(
      (sum, m) => sum + m.miles,
      0,
    );
    documents.push({
    ...
    ["schedule-c", "schedule-se", "home-office", "mileage-log"].forEach(
      (formId) => {
        const formNames = {
          "schedule-c": {
            name: "Schedule C",
            desc: "Profit or Loss",
            full: "Profit or Loss from Business",
          },
          "schedule-se": {
            name: "Schedule SE",
            desc: "Self-Employment Tax",
            full: "Self-Employment Tax Calculation",
          },
          "home-office": {
            name: "Home Office",
            desc: "Form 8829",
            full: "Expenses for Business Use of Your Home",
          },
          "mileage-log": {
            name: "Mileage Log",
            desc: "Vehicle Expenses",
            full: "Business Mileage and Vehicle Expense Log",
          },
        };
        documents.push({
          id: formId,
  ...
  documents.push({
    ...
    generatedData: null,
    note:
      investmentAccounts.length > 0 ? "Awaiting 1099-B from brokerage" : null,
  });

  // Schedule D - Capital Gains
  const capitalGains = investmentAccounts.reduce(
    (sum, acc) => sum + (acc.capitalGains || 0),
    0,
  );
  documents.push({
    id: "schedule-d",
  ...
  const iraContributions = iraAccounts.reduce(
    (sum, acc) => sum + (acc.contributions || 0),
    0,
  );
  documents.push({
    id: "5498",
  ...
  documents.push({
    ...
    generatedData: {
      ...
      recommendation:
        taxSummary.deductions.totalItemized > 14600 ? "Itemize" : "Standard",
    },
  });
  ...
  return (
    <span className={`status-badge ${config.className}`}>
      {Icon && (
        <Icon
          size={12}
          className={config.className === "generating" ? "spinning" : ""}
        />
      )}
  ...
  return (
    <div className={`document-card ${document.status.className}`}>
          ...
          <div className="doc-stats">
            ...
            {document.generatedData.itemCount !== undefined && (
              <span className="doc-items">
                {document.generatedData.itemCount} items
              </span>
            )}
          </div>
        )}
      ...
      <div className="doc-actions">
        {document.autoGenerate &&
          ...
          document.status !== DOC_STATUS.NOT_APPLICABLE && (
            <button
              ...
              title={
                document.status === DOC_STATUS.READY ? "Regenerate" : "Generate"
              }
            >
              <Sparkles size={14} />
            </button>
        ...
        {document.status === DOC_STATUS.GENERATING && (
          <button
            className="action-btn icon-only generating"
            disabled
            title="Generating..."
          >
  ...
  return (
    <div className="data-sources-panel">
      ...
      <p className="sources-note">
        Tax documents are auto-filled from your synced account data and
...
function EmptyState() {
  return (
    <div className="tax-docs-state empty">
      ...
      <p>
        Connect bank accounts and import transactions to auto-generate tax
        documents.
      </p>
...
export default function TaxDocuments() {
  ...
  const taxSummary = useMemo(
    () => calculateTaxSummary(selectedYear),
    [selectedYear],
  );
  ...
  const handleGenerate = async (docId) => {
    ...
    setDocuments((prev) =>
      prev.map((doc) =>
        doc.id === docId ? { ...doc, status: DOC_STATUS.GENERATING } : doc,
      ),
    );
  ...
  const handleGenerateAll = async () => {
    const autoGenDocs = documents.filter(
      (d) =>
        d.autoGenerate &&
  ...
  const groupedDocuments = categories.map((category) => ({
    category,
    icon:
      documents.find((d) => d.category === category)?.categoryIcon || FileText,
  ...
  const pendingCount = documents.filter(
    (d) => d.status === DOC_STATUS.PENDING_DATA,
  ).length;
  const generatingCount = documents.filter(
    (d) => d.status === DOC_STATUS.GENERATING,
  ).length;
    ...
    <div className="tax-documents-page">
      ...
      <div className="tax-advisory-banner">
        ...
        <div>
          ...
          <p>
            These documents are auto-generated from your {institutionCount}{" "}
            connected institutions and {accounts.length} synced accounts. Always
      ...
      <div className="tax-stats-bar">
        ...
        <div className="stat-item generating">
          <Loader2
            size={16}
            className={generatingCount > 0 ? "spinning" : ""}
          />
      ...
      <div className="document-categories">
        {groupedDocuments.map((category) => {
          ...
          const readyInCategory = category.documents.filter(
            (d) => d.status === DOC_STATUS.READY,
          ).length;
