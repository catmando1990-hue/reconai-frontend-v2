...
import { AlertTriangle, Calendar, CheckCircle2, ChevronRight, Download, FileText, Play, XCircle } from 'lucide-react';
...
function getTypeBadgeClass(type) {
  switch (type) {
    case 'Full ICS': return 'gcr-type-badge gcr-type-full-ics';
    case 'Labor': return 'gcr-type-badge gcr-type-labor';
    case 'Indirect': return 'gcr-type-badge gcr-type-indirect';
    default: return 'gcr-type-badge';
  }
}
...
function getSeverityClass(severity) {
  switch (severity) {
    case 'High': return 'gcr-severity-high';
    case 'Medium': return 'gcr-severity-medium';
    case 'Low': return 'gcr-severity-low';
    default: return '';
  }
}
...
function getSeverityLabel(severity) {
  switch (severity) {
    case 'High': return 'gcr-severity-label gcr-severity-label-high';
    case 'Medium': return 'gcr-severity-label gcr-severity-label-medium';
    case 'Low': return 'gcr-severity-label gcr-severity-label-low';
    default: return 'gcr-severity-label';
  }
}
...
export default function GovConReconciliation() {
  ...
  const [kpis, setKpis] = useState({ lastRun: '—', openCount: 0, resolvedCount: 0 });
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      // Normalize runs
      const runs = data.runs ?? data.reconciliation_runs ?? data.reconciliationRuns ?? [];
      ...
      const variances = data.open_variances ?? data.openVariances ?? data.variances ?? [];
      setOpenVariances(
        variances.map((v) => ({
          id: v.id,
          title: v.title ?? '',
      ...
      const schedules = data.ics_schedules ?? data.icsSchedules ?? data.schedules ?? [];
      setIcsSchedules(
        schedules.map((s) => ({
          code: s.code ?? '',
          name: s.name ?? '',
      ...
      setKpis({
        ...
        resolvedCount: data.resolved_count ?? data.resolvedCount ?? data.resolved_this_month ?? 0,
      });
    } catch (err) {
        ...
        <div className="gcr-main-content">
                ...
                <tbody>
                  {reconciliationRuns.map((run, index) => (
                    <tr key={index} className="gcr-table-row">
                      ...
                      <td>
                        <span className={getTypeBadgeClass(run.type)}>{run.type}</span>
                      </td>
                      <td>{run.period}</td>
                      ...
                      <td>
                        <span className={getStatusBadgeClass(run.status)}>
                          {run.status === 'Open' ? <XCircle size={12} /> : <CheckCircle2 size={12} />}
                          {run.status}
                        </span>
                      </td>
                    </tr>
                  ...
                  {reconciliationRuns.length === 0 && (
                    <tr>
                      <td colSpan="6" style={{ textAlign: 'center', color: '#888' }}>No reconciliation runs found.</td>
                    </tr>
                  )}
                </tbody>
              </table>
          ...
          <div className="gcr-card">
            <div className="gcr-card-header">
              ...
              <span className="gcr-variance-count">{openVariances.length} items</span>
            </div>
            <div className="gcr-variances-list">
              {openVariances.map((variance) => (
                <div key={variance.id} className={`gcr-variance-item ${getSeverityClass(variance.severity)}`}>
                  <div className="gcr-variance-header">
                    <span className="gcr-variance-title">{variance.title}</span>
                    <span className={getSeverityLabel(variance.severity)}>{variance.severity}</span>
                  </div>
              ...
              {openVariances.length === 0 && (
                <p style={{ color: '#888', padding: '1rem', textAlign: 'center' }}>No open variances.</p>
              )}
            </div>
          </div>
        </div>
          ...
          <div className="gcr-card">
            ...
            <ul className="gcr-schedule-list">
              {icsSchedules.map((schedule, index) => (
                <li key={index} className="gcr-schedule-item">
                  <div className="gcr-schedule-check">
                    {schedule.documented ? (
                      <CheckCircle2 size={16} className="gcr-check-icon gcr-check-documented" />
                    ) : (
                      <XCircle size={16} className="gcr-check-icon gcr-check-pending" />
                    )}
                  </div>
              ...
              {icsSchedules.length === 0 && (
                <li style={{ color: '#888', padding: '0.5rem 0' }}>No ICS schedules available.</li>
              )}
            </ul>
          </div>
        </div>
