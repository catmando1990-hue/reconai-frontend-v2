...
import { CheckCircle, ChevronLeft, ChevronRight, Clock, FileText, Percent, Send } from 'lucide-react';
...
export default function GovConTimekeeping() {
  ...
  const [kpis, setKpis] = useState({ totalHours: 0, utilization: '—', pendingApprovals: 0 });
  ...
  const fetchTimesheets = useCallback(async () => {
    try {
      ...
      const rows = payload.timesheets ?? payload.rows ?? (Array.isArray(payload) ? payload : []);
      setTimesheetData(
        rows.map((row) => ({
      ...
      const recent = payload.recent_submissions ?? payload.recentSubmissions ?? [];
      ...
      setKpis({
        ...
        utilization: payload.billable_utilization ?? payload.billableUtilization ?? payload.utilization ?? '—',
    ...
    <div className="govcon-timekeeping">
      ...
      <div className="gct-header">
        <div className="gct-header-left">
          ...
          <div className="gct-week-selector">
            ...
            <span className="gct-week-label">Week of Mar 24 - Mar 28, 2026</span>
      ...
      <div className="gct-kpi-grid">
        <div className="gct-kpi-card">
          ...
          <div className="gct-kpi-content">
            <span className="gct-kpi-value">{kpis.totalHours || weekTotal}</span>
        ...
        <div className="gct-kpi-card">
          ...
          <div className="gct-kpi-content">
            <span className="gct-kpi-value">{typeof kpis.utilization === 'number' ? `${kpis.utilization}%` : kpis.utilization}</span>
      ...
      <div className="gct-timesheet-card">
        <div className="gct-timesheet-header">
          ...
          <span className="gct-status-badge gct-status-draft">Draft - Not Submitted</span>
        </div>
          ...
          <table className="gct-table">
            <thead>
              <tr>
                ...
                {dayLabels.map((day) => (
                  <th key={day} className="gct-th gct-th-day">{day}</th>
                ))}
              ...
              {timesheetData.map((row, idx) => {
                ...
                return (
                  <tr key={idx} className="gct-row">
                    ...
                    <td className="gct-td gct-td-row-total">{formatHours(rowTotal)}</td>
                  </tr>
                );
              })}
              {timesheetData.length === 0 && (
                <tr>
                  <td colSpan={7 + dayLabels.length} style={{ textAlign: 'center', color: '#888' }}>
            ...
            {timesheetData.length > 0 && (
              <tfoot>
                <tr className="gct-totals-row">
                  <td className="gct-td gct-td-totals-label" colSpan={2}>Daily Totals</td>
                  {dailyTotals.map((total, idx) => (
                    <td key={idx} className="gct-td gct-td-daily-total">
                      {formatHours(total)}
                    </td>
                  ))}
                  <td className="gct-td gct-td-grand-total">{formatHours(weekTotal)}</td>
                </tr>
              </tfoot>
            )}
          </table>
      ...
      <div className="gct-recent-card">
        ...
        <ul className="gct-recent-list">
          ...
          {recentSubmissions.length === 0 && (
            <li style={{ color: '#888', padding: '0.5rem 0' }}>No recent submissions.</li>
          )}
        </ul>
      </div>
    </div>
