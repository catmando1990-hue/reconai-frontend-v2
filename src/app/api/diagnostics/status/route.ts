 ...
 *
 * Proxies diagnostic status requests to backend.
...
export async function GET() {
  ...
  try {
    ...
    // Proxy to backend
    const backendRes = await fetch(`${API_URL}/api/diagnostics/status`, {
      method: "GET",
      headers: {
    ...
    console.error("Diagnostic status proxy error:", error);
    return NextResponse.json(
      {
        error: {
          code: "PROXY_ERROR",
