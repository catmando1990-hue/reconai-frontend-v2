import { NextResponse } from "next/server";
import { auth } from "@clerk/nextjs/server";

// Proxy to Render backend
const BACKEND_URL =
  process.env.NEXT_PUBLIC_API_BASE_URL ||
  "https://reconai-backend.onrender.com";

export async function GET() {
  try {
    const { userId, getToken } = await auth();
    if (!userId) {
      return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
    }

    const token = await getToken();

    const resp = await fetch(`${BACKEND_URL}/api/intelligence/status`, {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
        ...(token ? { Authorization: `Bearer ${token}` } : {}),
      },
    });

    if (!resp.ok) {
      const data = await resp.json().catch(() => ({}));
      return NextResponse.json(
        {
          error:
            data.detail || data.error || "Failed to fetch intelligence status",
        },
        { status: resp.status },
      );
    }

    const data = await resp.json();

    // Transform backend response to match frontend IntelData interface
    // Backend returns: { ok, enabled, mode, manualRunRequired, confidenceThreshold, categories, lastRun, cache }
    // Frontend expects: { lastRun, cache }
    const intelStatus = {
      lastRun: data.lastRun || undefined,
      cache: data.cache || undefined,
    };

    return NextResponse.json(intelStatus);
  } catch (err: unknown) {
    console.error("Intelligence status fetch error:", err);
    const message = err instanceof Error ? err.message : "Unknown error";
    return NextResponse.json(
      { error: "Failed to fetch intelligence status: " + message },
      { status: 500 },
    );
  }
}
