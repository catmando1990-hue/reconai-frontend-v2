...
export async function GET() {
  ...
  try {
    ...
    console.log(
      `[Plaid accounts] Query: userId=${userId}, orgId=${orgId}, requestId=${requestId}`,
    );
    ...
    console.log(
      `[Plaid accounts] Items check: count=${items?.length ?? 0}, error=${itemsError?.message ?? "none"}`,
    );
    if (items && items.length > 0) {
      console.log(
        `[Plaid accounts] Items found:`,
        items.map((i) => ({
          item_id: i.item_id,
    ...
    console.log(
      `[Plaid accounts] Result: count=${accounts?.length ?? 0}, error=${error?.message ?? "none"}, requestId=${requestId}`,
    );
        ...
        if (backendUrl) {
          ...
          if (res.ok) {
            ...
            if (Array.isArray(backendAccounts) && backendAccounts.length > 0) {
              ...
              const normalized = backendAccounts.map((acct: any) => ({
                ...
                name: acct.name ?? null,
                official_name: acct.official_name ?? acct.officialName ?? null,
                type: acct.type ?? null,
