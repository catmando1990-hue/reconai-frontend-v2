...
type ReconciliationStatus = "pending" | "in_progress" | "completed" | "approved" | "failed";
...
const ICS_SCHEDULES = [
  ...
  { schedule: "I", name: "Cumulative Allowable Cost Worksheet", status: "in_progress" },
  { schedule: "J", name: "Subcontract Information", status: "pending" },
  { schedule: "K", name: "Summary of Hours and Amounts", status: "complete" },
  { schedule: "L", name: "Reconciliation of Contract Briefs", status: "in_progress" },
...
export default function ReconciliationPage() {
  ...
  const [varianceDisplayCount, setVarianceDisplayCount] = useState(INITIAL_VARIANCE_COUNT);
  ...
  const { totalVariances, resolvedVariances, escalatedVariances } = useMemo(() => ({
    ...
    resolvedVariances: variances.filter((v) => v.status === "resolved").length,
    escalatedVariances: variances.filter((v) => v.status === "escalated").length,
  }), [variances]);
  ...
  const loadMoreVariances = useCallback(() => {
    setVarianceDisplayCount((prev) => Math.min(prev + LOAD_MORE_COUNT, allRunVariances.length));
  ...
  return (
    <main className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            DCAA-compliant labor and indirect cost reconciliation with ICS preparation
          </p>
        </div>
      ...
      <div className="flex items-start gap-3 p-4 rounded-lg bg-blue-500/10 border border-blue-500/20">
        ...
        <div>
          <p className="text-sm font-medium text-blue-500">Incurred Cost Submission Requirements</p>
          <p className="text-sm text-muted-foreground">
            Reconciliation supports DCAA Incurred Cost Submission (ICS) per FAR 52.216-7.
            All variances must be resolved with documented evidence before final submission.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="rounded-xl border bg-card p-4">
          ...
          <p className="mt-2 text-2xl font-semibold text-green-500">{resolvedVariances}</p>
        </div>
        <div className="rounded-xl border bg-card p-4">
          <div className="flex items-center gap-2 text-muted-foreground">
          ...
          <p className="mt-2 text-2xl font-semibold text-red-500">{escalatedVariances}</p>
        </div>
      </div>
        ...
        <div className="divide-y">
          {runs.map((run) => (
            <div
              ...
              onClick={() => handleRunSelect(selectedRun?.id === run.id ? null : run)}
            >
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-6">
                  ...
                  <div className="text-right">
                    ...
                    <p className={`font-medium ${run.variances_found > 0 ? "text-yellow-500" : "text-green-500"}`}>
                      {run.variances_found}
                    </p>
                  </div>
                  <div className="text-right">
                    <p className="text-sm text-muted-foreground">Resolved</p>
                    <p className="font-medium text-green-500">{run.variances_resolved}</p>
                  </div>
        ...
        <div className="p-4 border-b flex items-center justify-between">
          <div>
            <h2 className="font-medium">
              {selectedRun ? `Variances - ${selectedRun.period}` : "All Variances"}
            </h2>
            <p className="text-sm text-muted-foreground">
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y">
              {runVariances.map((variance) => (
                <tr key={variance.id} className="hover:bg-muted/50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-right">
                    <span
                      className={`font-mono ${
                        variance.variance_amount > 0 ? "text-red-500" : "text-green-500"
                      }`}
                    >
                  ...
                  <td className="px-4 py-3 text-sm">{variance.assigned_to || "-"}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        {hasMoreVariances && (
          <div className="p-4 border-t flex justify-center">
            ...
            >
              ...
              Load More ({allRunVariances.length - varianceDisplayCount} remaining)
            </button>
          </div>
        )}
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 p-4">
          {ICS_SCHEDULES.map((schedule) => (
            <div key={schedule.schedule} className="p-3 rounded-lg bg-muted/50">
              <div className="flex items-center justify-between">
                <span className="font-mono text-sm font-medium">Schedule {schedule.schedule}</span>
                <span className={`text-xs ${getScheduleStatusColor(schedule.status)}`}>
                  {schedule.status === "complete" && <CheckCircle className="h-4 w-4 inline mr-1" />}
                  {schedule.status === "in_progress" && <Clock className="h-4 w-4 inline mr-1" />}
                  {schedule.status}
                </span>
              </div>
              <p className="text-sm text-muted-foreground mt-1">{schedule.name}</p>
            </div>
          ))}
        </div>
      </div>
