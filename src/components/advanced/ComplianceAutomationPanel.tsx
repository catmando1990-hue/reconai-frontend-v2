...
export function ComplianceAutomationPanel({ apiBase }: { apiBase: string }) {
  ...
  const [activeTab, setActiveTab] = React.useState<'frameworks' | 'controls' | 'gaps'>('frameworks');
  ...
  const [dcaaStatus, setDcaaStatus] = React.useState<ComplianceStatus | null>(null);
  ...
  const fetchFrameworks = React.useCallback(async () => {
    ...
    setErr(null);
    try {
      const res = await fetch(`${apiBase}/api/compliance/frameworks`, { credentials: 'include' });
  ...
  const fetchControls = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${apiBase}/api/compliance/sf1408/mappings`, { credentials: 'include' });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const json = await res.json();
  ...
  const fetchGaps = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${apiBase}/api/compliance/gaps?framework=sf1408`, { credentials: 'include' });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const json = await res.json();
      setGaps(json.gaps || []);
      setRequestId(json.request_id);
  ...
  const fetchDcaaStatus = async () => {
    try {
      const res = await fetch(`${apiBase}/api/compliance/dcaa/status`, { credentials: 'include' });
      if (!res.ok) return;
      const json = await res.json();
  ...
  const collectEvidence = async () => {
    ...
    try {
      const res = await fetch(`${apiBase}/api/compliance/evidence/collect`, {
        ...
        body: JSON.stringify({ framework: 'sf1408', collection_type: 'manual' }),
      });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const json = await res.json();
      setRequestId(json.request_id);
    ...
    <div className="rounded-2xl border p-4">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <div className="text-xs opacity-70">DCAA/SF-1408 compliance tracking and gap analysis (read-only).</div>
        </div>
        <button
          type="button"
      ...
      {dcaaStatus ? (
        <div className="mt-4 rounded-xl border p-3">
          <div className="flex items-center justify-between">
            ...
            <span className={`px-2 py-1 rounded text-sm ${getStatusColor(dcaaStatus.overall_status)}`}>
              {dcaaStatus.compliance_score}%
            </span>
          </div>
          <div className="mt-2 grid grid-cols-4 gap-2 text-xs">
            <div className="text-center">
              <div className="font-bold text-green-600">{dcaaStatus.sf1408_compliance.compliant}</div>
              <div className="opacity-70">Compliant</div>
            </div>
            <div className="text-center">
              <div className="font-bold text-yellow-600">{dcaaStatus.sf1408_compliance.partial}</div>
              <div className="opacity-70">Partial</div>
            </div>
            <div className="text-center">
              <div className="font-bold text-red-600">{dcaaStatus.sf1408_compliance.non_compliant}</div>
              <div className="opacity-70">Non-Compliant</div>
            </div>
            <div className="text-center">
              <div className="font-bold">{dcaaStatus.sf1408_compliance.total_controls}</div>
      ...
      {loading ? (
        ...
        <div className="mt-3 grid gap-2">
          {frameworks.map((f) => (
            <div key={f.id} className="rounded-lg border p-3">
              ...
              <div className="text-xs mt-1 opacity-50">{f.controls_count} controls</div>
            </div>
          ))}
        </div>
      ) : activeTab === 'controls' ? (
        ...
        <div className="mt-3 grid gap-2">
          {gaps.length === 0 ? (
            <div className="text-sm opacity-70">No compliance gaps identified.</div>
          ) : (
            gaps.map((g) => (
              <div key={g.control_id} className="rounded-lg border border-yellow-200 p-3">
                <div className="flex justify-between items-start">
                  <div>
                    <span className="font-mono text-xs opacity-50">{g.control_id}</span>
                    <span className="ml-2 text-sm font-medium">{g.control_name}</span>
                  </div>
                  <span className={`px-2 py-0.5 rounded text-xs ${g.severity === 'high' ? 'bg-red-100 text-red-800' : 'bg-yellow-100 text-yellow-800'}`}>
                    {g.severity}
                  </span>
                </div>
                <div className="text-xs mt-2 opacity-70">{g.remediation}</div>
                <div className="text-xs mt-1 opacity-50">Effort: {g.estimated_effort}</div>
              </div>
            ))
          )}
        </div>
