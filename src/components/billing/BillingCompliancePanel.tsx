...
export function BillingCompliancePanel({ apiBase }: { apiBase: string }) {
  ...
  const fetchPolicy = React.useCallback(async () => {
    ...
    setErr(null);
    try {
      const res = await fetch(`${apiBase}/api/billing/retention/policy`, { credentials: 'include' });
      if (!res.ok) {
      ...
      const message = e instanceof Error ? e.message : 'Failed to load retention policy';
      setErr(message);
    } finally {
      setLoading(false);
    }
  ...
  const requestExport = async () => {
    ...
    try {
      ...
      const message = e instanceof Error ? e.message : 'Failed to request export';
      setErr(message);
    } finally {
      setProcessing(false);
    }
  ...
  const requestDelete = async () => {
    ...
    if (!deleteReason || deleteReason.length < 10) {
      setErr('Please provide a reason (at least 10 characters) for the deletion request.');
      return;
    }
    ...
    try {
      ...
      const message = e instanceof Error ? e.message : 'Failed to request deletion';
      setErr(message);
    } finally {
      setProcessing(false);
    }
    ...
    <div className="rounded-2xl border p-4">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <div className="text-xs opacity-70">Right-to-export and right-to-delete with immutable audit sealing.</div>
        </div>
        <button
          type="button"
          onClick={fetchPolicy}
      ...
      <div className="mt-4 rounded-xl border p-3">
        ...
        <div className="grid gap-3">
          <div>
            <label className="text-xs opacity-70 block mb-1">Retention Period (days)</label>
            <input
              type="number"
              min="30"
              max="3650"
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              className="rounded"
            />
            <label htmlFor="autoDelete" className="text-sm">Enable auto-delete after retention period</label>
          </div>
          <button
            type="button"
            onClick={savePolicy}
        ...
        {policy ? (
          <div className="mt-2 text-xs opacity-50">Request ID: {policy.request_id}</div>
        ) : null}
      </div>
      ...
      <div className="mt-4 rounded-xl border p-3">
        ...
        {exportResult ? (
          <div className="mt-3 rounded-lg border p-2 text-sm">
            <div className="flex justify-between"><span className="opacity-70">Status</span><span>{exportResult.status}</span></div>
            <div className="flex justify-between"><span className="opacity-70">Records</span><span>{exportResult.record_count ?? 0}</span></div>
            {exportResult.audit_seal ? (
              <div className="flex justify-between"><span className="opacity-70">Audit Seal</span><span className="font-mono text-xs truncate max-w-[200px]">{exportResult.audit_seal}</span></div>
            ) : null}
            <div className="flex justify-between"><span className="opacity-70">Request ID</span><span className="font-mono text-xs">{exportResult.request_id}</span></div>
          </div>
        ) : null}
      </div>
      ...
      <div className="mt-4 rounded-xl border border-red-200 p-3">
        <div className="text-sm font-medium mb-3 text-red-600">Right to Delete (Destructive)</div>
        <div className="grid gap-3">
          <div className="text-xs text-red-500">
            Warning: This action is irreversible. All billing data will be permanently deleted and an immutable audit record will be created.
          </div>
          <div>
            <label className="text-xs opacity-70 block mb-1">Reason for deletion (required, min 10 chars)</label>
            <input
              type="text"
              value={deleteReason}
          ...
          <button
            ...
            disabled={processing || deleteConfirm !== 'DELETE MY DATA' || deleteReason.length < 10}
        ...
        {deleteResult ? (
          <div className="mt-3 rounded-lg border border-red-200 p-2 text-sm">
            <div className="flex justify-between"><span className="opacity-70">Status</span><span>{deleteResult.status}</span></div>
            <div className="flex justify-between"><span className="opacity-70">Records Deleted</span><span>{deleteResult.records_deleted}</span></div>
            <div className="flex justify-between"><span className="opacity-70">Audit Seal</span><span className="font-mono text-xs truncate max-w-[200px]">{deleteResult.audit_seal}</span></div>
            <div className="flex justify-between"><span className="opacity-70">Immutable Log</span><span className="font-mono text-xs">{deleteResult.immutable_log_id}</span></div>
            <div className="flex justify-between"><span className="opacity-70">Request ID</span><span className="font-mono text-xs">{deleteResult.request_id}</span></div>
          </div>
        ) : null}
      </div>
    </div>
