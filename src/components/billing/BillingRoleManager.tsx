...
export function BillingRoleManager({ apiBase }: { apiBase: string }) {
  ...
  const fetchRoles = React.useCallback(async () => {
    ...
    setErr(null);
    try {
      const res = await fetch(`${apiBase}/api/billing/roles`, { credentials: 'include' });
      if (!res.ok) {
      ...
      const message = e instanceof Error ? e.message : 'Failed to load billing roles';
      setErr(message);
    } finally {
      setLoading(false);
    }
  ...
  const saveChanges = async () => {
    ...
    try {
      ...
      if (failures.length > 0) {
        setErr(`Some updates failed: ${failures.map(f => f.error).join(', ')}`);
      }
    ...
    <div className="rounded-2xl border p-4">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <div className="text-xs opacity-70">Manage billing permissions for team members (manual).</div>
        </div>
        ...
        <div className="mt-4 space-y-2">
          {roles.map((member) => (
            <div key={member.user_id} className="flex items-center justify-between gap-3 rounded-xl border p-3">
              ...
              {member.is_owner ? (
                <span className="text-xs px-2 py-1 rounded bg-gray-200">Owner (cannot change)</span>
              ) : (
                <select
