...
export default function Sidebar({
  ...
  const tiers: Tier[] = useMemo(() => {
    const core: NavItem[] = [
      { name: 'Overview', href: '/dashboard', icon: LayoutDashboard, section: 'Core' },

      { name: 'Accounts', href: '/accounts', icon: Building2, section: 'Core' },
      { name: 'Transactions', href: '/transactions', icon: ArrowLeftRight, section: 'Core' },
      ...
      { name: 'Maintenance', href: '/maintenance', icon: Wrench, section: 'Core' },
    ];

    const intelligence: NavItem[] = [
      { name: 'AI Worker', href: '/ai-worker', icon: Bot, section: 'Intelligence' },
      { name: 'Alerts', href: '/alerts', icon: Bell, section: 'Intelligence' },
      { name: 'Insights', href: '/insights', icon: Sparkles, section: 'Intelligence' },
    ];

    const cfo: NavItem[] = [
      { name: 'Analyze', href: '/financial-reports', icon: BarChart3, section: 'CFO Mode' },
      { name: 'Cash Flow', href: '/cash-flow', icon: PieChart, section: 'CFO Mode' },

      // Compliance: implied posture—kept available but de-emphasized
      { name: 'Security & posture', href: '/security', icon: ShieldCheck, section: 'CFO Mode' },
      { name: 'Compliance', href: '/compliance', icon: ShieldCheck, section: 'CFO Mode' },
      { name: 'Certifications', href: '/certifications', icon: ShieldCheck, section: 'CFO Mode' },
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between px-4 py-4 border-b border-border">
        <div className={cx('flex items-center gap-3', isCollapsed && 'justify-center w-full')}>
      ...
      {/* Tiers */}
      <div className={cx('px-3 py-4 space-y-2 flex-1 overflow-y-auto', isCollapsed && 'px-2')}>
        {tiers.map((tier) => {
          const isOpen = openTier === tier.id;
          const anyActive = tier.items.some((it) => isActive(pathname, it.href));

          return (
            <div key={tier.id} className="rounded-2xl border border-border bg-card/40">
              <button
                type="button"
              ...
              >
                ...
                {!isCollapsed && (
                  <ChevronDown className={cx('h-4 w-4 text-muted-foreground transition', isOpen && 'rotate-180')} />
                )}
              </button>
                ...
                <div className="px-1 pb-2 grid grid-cols-1 gap-1">
                  {tier.items.slice(0, 4).map((item) => {
                    ...
                    return (
                      ...
                      >
                        <Icon className={cx('h-4 w-4', active ? 'text-primary' : 'text-muted-foreground')} />
                      </Link>
                    );
                  })}
                </div>
      ...
      <div className="border-t border-border bg-background p-3">
        {!isCollapsed && (
          <div className="mb-3 rounded-2xl border border-border bg-card p-3">
            <div className="text-sm font-semibold">{user?.fullName ?? 'Signed in'}</div>
