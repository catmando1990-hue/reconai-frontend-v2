...
export function OnboardingPanel({ apiBase }: { apiBase: string }) {
  ...
  const [checklistData, setChecklistData] = React.useState<ChecklistResponse | null>(null);
  const [insightsData, setInsightsData] = React.useState<InsightsResponse | null>(null);
  const [activeTab, setActiveTab] = React.useState<'progress' | 'checklist' | 'sample-data'>('progress');
  ...
  const fetchData = React.useCallback(async () => {
    ...
    try {
      const [statusRes, checklistRes, insightsRes] = await Promise.all([
        ...
        fetch(`${apiBase}/api/onboarding/checklist`, { credentials: 'include' }),
        fetch(`${apiBase}/api/onboarding/first-run-insights`, { credentials: 'include' }),
      ]);

      if (!statusRes.ok) throw new Error(`Status: HTTP ${statusRes.status}`);
      if (!checklistRes.ok) throw new Error(`Checklist: HTTP ${checklistRes.status}`);
    ...
    <div className="rounded-2xl border p-4">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <div className="text-xs opacity-70">Setup progress, checklists, sample data seeding.</div>
        </div>
        <button
          type="button"
          onClick={fetchData}
        ...
        <div className="mt-4">
            ...
            <div className="relative h-32 w-32">
              <svg className="h-32 w-32 -rotate-90" viewBox="0 0 100 100">
                ...
                <circle
                  ...
                  r="45"
                  fill="none"
                  stroke={statusData.progress.is_complete ? '#22c55e' : '#3b82f6'}
                  strokeWidth="10"
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span className="text-2xl font-bold">{statusData.progress.completion_percent}%</span>
          ...
          <div className="mt-4 grid grid-cols-2 gap-3">
            <div className="rounded-xl border p-3 text-center">
              <div className="text-xl font-bold">{statusData.progress.completed_steps}/{statusData.progress.total_steps}</div>
              <div className="text-xs opacity-70">Steps Completed</div>
            </div>
            <div className="rounded-xl border p-3 text-center">
              <div className="text-xl font-bold">{statusData.progress.required_completed}/{statusData.progress.required_steps}</div>
          ...
          {checklistData.next_step ? (
            <div className="mb-4 rounded-xl border-2 border-blue-500 bg-blue-50 p-3">
              <div className="flex items-center gap-2">
                ...
                <span className="font-medium">Next: {checklistData.next_step.name}</span>
              </div>
              <div className="mt-1 text-xs opacity-70">{checklistData.next_step.description}</div>
              <div className="mt-2 flex items-center justify-between">
                <span className="text-xs opacity-70">{checklistData.next_step.estimated_time}</span>
                <button
                  type="button"
                ...
                >
                  {completing === checklistData.next_step.id ? 'Completing...' : 'Mark Complete'}
                </button>
              </div>
            </div>
          ) : null}
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    {step.required ? (
                      <span className="rounded bg-red-100 px-1 text-xs text-red-700">Required</span>
                    ) : null}
                  </div>
                  {!step.completed ? (
                    <button
                ...
                <div className="ml-5 mt-1 text-xs opacity-70">{step.description}</div>
                <div className="ml-5 mt-1 text-xs opacity-50">{step.estimated_time}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="mt-4">
            ...
            {insightsData.sample_data_templates.map((template) => (
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs opacity-70">{template.transaction_count} transactions</span>
                </div>
                <div className="mt-1 text-xs opacity-70">{template.description}</div>
                <div className="mt-2 flex flex-wrap gap-1">
                  {template.categories.map((cat) => (
                    <span key={cat} className="rounded bg-gray-100 px-1 py-0.5 text-xs">{cat}</span>
                  ))}
                </div>
              </div>
            ))}
          ...
          {selectedTemplate ? (
            <div className="mt-4 rounded-xl border p-3">
              <div className="text-sm font-medium">Confirm Sample Data Seed</div>
      ...
      {statusData ? (
        <div className="mt-3 text-xs opacity-50">Request ID: {statusData.request_id}</div>
      ) : null}
    </div>
  );
}
