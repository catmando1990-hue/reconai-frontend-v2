...
export function OrgGovernancePanel({ apiBase }: { apiBase: string }) {
  ...
  const [snapshot, setSnapshot] = React.useState<GovernanceSnapshot | null>(
    null,
  );
  const [compliance, setCompliance] = React.useState<
    ComplianceResponse["compliance"] | null
  >(null);
  const [requestId, setRequestId] = React.useState<string | null>(null);
  const [activeTab, setActiveTab] = React.useState<
    "overview" | "compliance" | "access" | "data"
  >("overview");
  ...
  const fetchData = React.useCallback(async () => {
    ...
    try {
      const [snapshotRes, complianceRes] = await Promise.all([
        fetch(`${apiBase}/api/org/governance/snapshot`, {
          credentials: "include",
        }),
        fetch(`${apiBase}/api/org/governance/compliance`, {
          credentials: "include",
        }),
      ]);
      ...
      if (!complianceRes.ok)
        throw new Error(`Compliance: HTTP ${complianceRes.status}`);
        ...
        <div className="mt-4 space-y-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div>
                ...
                <div className="font-medium capitalize">
                  {snapshot.organization.tier_name}
                </div>
              </div>
              <div>
                <div className="text-xs opacity-70">Users</div>
                <div className="font-medium">
                  {snapshot.organization.user_count}
                </div>
              </div>
              <div>
                <div className="text-xs opacity-70">Created</div>
                <div className="text-sm">
                  {formatTimestamp(snapshot.organization.created_at)}
                </div>
              </div>
              <div>
                <div className="text-xs opacity-70">Org ID</div>
                <div className="font-mono text-xs">
                  {snapshot.organization.id.slice(0, 8)}...
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <div className="rounded-xl border p-3 text-center">
              <div
                className={`inline-block px-2 py-1 rounded text-xs ${getStatusColor(snapshot.compliance.dcaa_status)}`}
              >
            ...
            <div className="rounded-xl border p-3 text-center">
              <div
                className={`inline-block px-2 py-1 rounded text-xs ${getStatusColor(snapshot.compliance.soc2_status)}`}
              >
            ...
            <div className="rounded-xl border p-3 text-center">
              <div
                className={`inline-block px-2 py-1 rounded text-xs ${getStatusColor(snapshot.access_controls.mfa_enforced ? "enabled" : "not_applicable")}`}
              >
                {snapshot.access_controls.mfa_enforced
                  ? "Enforced"
                  : "Optional"}
              </div>
            ...
            <div className="rounded-xl border p-3 text-center">
              <div
                className={`inline-block px-2 py-1 rounded text-xs ${getStatusColor(snapshot.data_policies.encryption_at_rest ? "enabled" : "not_applicable")}`}
              >
                {snapshot.data_policies.encryption_at_rest
                  ? "Active"
                  : "Inactive"}
              </div>
        ...
        <div className="mt-4 space-y-4">
          <div className="rounded-xl border p-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <span
                className={`px-2 py-1 rounded text-xs ${getStatusColor(compliance.overall_status)}`}
              >
              ...
              {compliance.frameworks.map((framework) => (
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <span
                      className={`px-2 py-1 rounded text-xs ${getStatusColor(framework.status)}`}
                    >
                  ...
                  <div className="mt-3">
                    <div className="h-2 bg-gray-200 rounded-full overflow-hidden">
                      <div
                        ...
                        style={{
                          width: `${(framework.controls_met / framework.controls_total) * 100}%`,
                        }}
                      />
                    </div>
                  </div>
          ...
          <div className="rounded-xl border p-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="rounded-lg border p-3">
                ...
                <div
                  className={`mt-1 text-xs ${snapshot.access_controls.rbac_enabled ? "text-green-600" : "text-gray-500"}`}
                >
                  {snapshot.access_controls.rbac_enabled
                    ? "Enabled"
                    : "Disabled"}
                </div>
              </div>
              <div className="rounded-lg border p-3">
                ...
                <div
                  className={`mt-1 text-xs ${snapshot.access_controls.mfa_enforced ? "text-green-600" : "text-gray-500"}`}
                >
                  {snapshot.access_controls.mfa_enforced
                    ? "Enforced"
                    : "Optional"}
              ...
              <div className="rounded-lg border p-3">
                ...
                <div className="mt-1 text-xs">
                  {snapshot.access_controls.session_timeout_minutes} minutes
                </div>
              </div>
              <div className="rounded-lg border p-3">
                <div className="text-sm font-medium">IP Allowlist</div>
                <div
                  className={`mt-1 text-xs ${snapshot.access_controls.ip_allowlist_enabled ? "text-green-600" : "text-gray-500"}`}
                >
                  {snapshot.access_controls.ip_allowlist_enabled
                    ? "Enabled"
                    : "Not Available"}
                </div>
              </div>
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between rounded-lg border p-3">
                ...
                <div className="text-sm font-medium">
                  {snapshot.data_policies.retention_days} days
                </div>
              </div>
              <div className="flex items-center justify-between rounded-lg border p-3">
                <div>
                ...
                <span
                  className={`px-2 py-1 rounded text-xs ${snapshot.data_policies.export_enabled ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-500"}`}
                >
                  {snapshot.data_policies.export_enabled
                    ? "Enabled"
                    : "Disabled"}
                </span>
              </div>
              <div className="flex items-center justify-between rounded-lg border p-3">
                ...
                <span
                  className={`px-2 py-1 rounded text-xs ${snapshot.data_policies.deletion_enabled ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-500"}`}
                >
                  {snapshot.data_policies.deletion_enabled
                    ? "Enabled"
                    : "Disabled"}
                </span>
              </div>
              <div className="flex items-center justify-between rounded-lg border p-3">
                ...
                <span
                  className={`px-2 py-1 rounded text-xs ${snapshot.data_policies.encryption_at_rest ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}
                >
                  {snapshot.data_policies.encryption_at_rest
                    ? "Active"
                    : "Inactive"}
              ...
              <div className="flex items-center justify-between rounded-lg border p-3">
                ...
                <span
                  className={`px-2 py-1 rounded text-xs ${snapshot.data_policies.encryption_in_transit ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}
                >
                  {snapshot.data_policies.encryption_in_transit
                    ? "Active"
                    : "Inactive"}
                </span>
              </div>
