...
export function ProductionReadinessPanel({ apiBase }: { apiBase: string }) {
  ...
  const [runbooksData, setRunbooksData] =
    React.useState<RunbooksResponse | null>(null);
  const [healthChecksData, setHealthChecksData] =
    React.useState<HealthChecksResponse | null>(null);
  const [activeTab, setActiveTab] = React.useState<
    "slos" | "budget" | "runbooks" | "health"
  >("slos");
  const [selectedRunbook, setSelectedRunbook] = React.useState<Runbook | null>(
    null,
  );
  ...
  const fetchData = React.useCallback(async () => {
    ...
    try {
      const [slosRes, budgetRes, runbooksRes, healthRes] = await Promise.all([
        ...
        fetch(`${apiBase}/api/production/health-checks`, {
          credentials: "include",
        }),
      ]);
      ...
      if (!runbooksRes.ok)
        throw new Error(`Runbooks: HTTP ${runbooksRes.status}`);
      ...
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <div className="text-xs opacity-70">
            SLOs, error budgets, runbooks, health checks.
          </div>
        </div>
        <button
          type="button"
            ...
            {slosData.slos.map((slo) => (
              <div key={slo.id} className="rounded-lg border p-3">
                <div className="flex items-center justify-between">
                  ...
                  <span
                    className={`rounded-full px-2 py-0.5 text-xs ${slo.status === "met" ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}
                  >
                ...
                <div className="mt-1">
                  <div className="h-2 w-full rounded-full bg-gray-200">
                    <div
                      ...
                      style={{
                        width: `${Math.min((slo.current_value / slo.target) * 100, 100)}%`,
                      }}
                    />
                  </div>
                </div>
      ...
      {activeTab === "budget" && errorBudgetData ? (
        <div className="mt-4">
            ...
            <div className="mt-4 grid grid-cols-3 gap-3 text-center text-xs">
              <div>
                <div className="font-medium">
                  {errorBudgetData.error_budget.budget_total_minutes.toFixed(1)}
                </div>
          ...
          <div className="mt-3 text-xs opacity-70">
            Based on {errorBudgetData.slo_reference?.target}% availability SLO
            over 30 days
          </div>
        </div>
      ) : null}
      ...
      {activeTab === "runbooks" && runbooksData ? (
        <div className="mt-4">
          <div className="mb-3 text-xs opacity-70">
            {runbooksData.total_count} runbooks available
          </div>
          <div className="space-y-2">
            {runbooksData.runbooks.map((runbook) => (
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <span
                    className={`rounded-full px-2 py-0.5 text-xs ${runbook.severity === "P1" ? "bg-red-100 text-red-700" : "bg-yellow-100 text-yellow-700"}`}
                  >
                    {runbook.severity}
                  </span>
                </div>
      ...
      {activeTab === "health" && healthChecksData ? (
        <div className="mt-4">
          <div className="mb-3 flex items-center gap-2">
            <span
              className={`h-3 w-3 rounded-full ${healthChecksData.overall_status === "healthy" ? "bg-green-500" : "bg-yellow-500"}`}
            />
            <span className="text-sm font-medium capitalize">
              {healthChecksData.overall_status}
            </span>
          </div>
          <div className="space-y-2">
            {healthChecksData.health_checks.map((check) => (
              ...
              >
                <div className="flex items-center gap-2">
                  <span
                    className={`h-2 w-2 rounded-full ${check.status === "healthy" ? "bg-green-500" : "bg-yellow-500"}`}
                  />
          ...
          <div className="mt-4 rounded-xl border p-3">
            ...
            <div className="mt-2 text-xs opacity-70">
              Trigger a smoke load test (60s, 100 RPS)
            </div>
      ...
      {selectedRunbook ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="max-h-[80vh] w-full max-w-lg overflow-auto rounded-xl bg-white p-6">
            ...
            <div className="mt-2 flex gap-2">
              <span
                className={`rounded-full px-2 py-0.5 text-xs ${selectedRunbook.severity === "P1" ? "bg-red-100 text-red-700" : "bg-yellow-100 text-yellow-700"}`}
              >
