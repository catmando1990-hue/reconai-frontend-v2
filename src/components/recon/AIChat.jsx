...
// Confidence levels
const CONFIDENCE = {
  HIGH: { level: 'high', label: 'High Confidence', icon: CheckCircle, color: '#1abc9c' },
  MEDIUM: { level: 'medium', label: 'Medium Confidence', icon: AlertCircle, color: '#f39c12' },
  LOW: { level: 'low', label: 'Low Confidence', icon: AlertTriangle, color: '#e74c3c' },
  ADVISORY: { level: 'advisory', label: 'General Guidance', icon: Info, color: '#4680ff' },
};
...
// Advisory disclaimer
const DISCLAIMER = "This is general financial guidance, not professional advice. Please consult a qualified financial advisor for decisions about your specific situation.";
...
function generateResponse(userMessage) {
  ...
  if (lowerMsg.includes('deduction') || lowerMsg.includes('deduct') || lowerMsg.includes('write off') || lowerMsg.includes('tax break')) {
    return {
      content: "There are many tax deductions available depending on your situation. Here are common ones:\n\n**For Everyone:**\n• Standard deduction ($13,850 single / $27,700 married for 2024)\n• Student loan interest (up to $2,500)\n• Educator expenses (up to $300 for teachers)\n\n**If You Itemize:**\n• State and local taxes (SALT) - up to $10,000\n• Mortgage interest\n• Charitable donations\n• Medical expenses over 7.5% of AGI\n\n**Self-Employed:**\n• Home office deduction\n• Business equipment and supplies\n• Health insurance premiums\n• Self-employment tax (50% deductible)\n\n**Pro tip:** Keep receipts and records throughout the year!",
      confidence: CONFIDENCE.MEDIUM,
  ...
  // Greeting
  if (lowerMsg.includes('hello') || lowerMsg.includes('hi') || lowerMsg.includes('hey') || lowerMsg.match(/^hi$/)) {
    return {
      content: "Hey there! Great to chat with you. I'm your ReconAI assistant - I can help with financial questions, or we can just have a conversation.\n\nWhat's on your mind today?",
  ...
  if (lowerMsg.includes('thank') || lowerMsg.includes('thanks')) {
    return {
      content: "You're welcome! Happy to help anytime. Is there anything else you'd like to talk about?",
      confidence: CONFIDENCE.ADVISORY,
      category: RESPONSE_CATEGORIES.CONVERSATION,
      sources: [],
    };
  ...
  if (lowerMsg.includes('how are you') || lowerMsg.includes("how's it going") || lowerMsg.includes("what's up") || lowerMsg.includes('whats up')) {
    return {
      content: "I'm doing great, thanks for asking! Always ready to help or just chat. How about you - how's your day going?",
      confidence: CONFIDENCE.ADVISORY,
      category: RESPONSE_CATEGORIES.CONVERSATION,
      sources: [],
    };
  ...
  if (lowerMsg.includes('weather')) {
    return {
      content: "I wish I could check the weather for you! While I don't have access to live weather data, I can definitely help you plan financially for any weather-related expenses - like travel, seasonal activities, or emergency preparedness.\n\nIs there something specific I can help you with?",
      confidence: CONFIDENCE.ADVISORY,
      category: RESPONSE_CATEGORIES.CONVERSATION,
      sources: [],
    };
  ...
  // Fun fact
  if (lowerMsg.includes('fun fact') || lowerMsg.includes('interesting fact') || lowerMsg.includes('tell me something')) {
    const funFacts = [
  ...
  if (lowerMsg.includes('good morning') || lowerMsg.includes('good afternoon') || lowerMsg.includes('good evening') || lowerMsg.includes('good night')) {
    const timeGreeting = lowerMsg.includes('morning') ? 'morning' :
                         lowerMsg.includes('afternoon') ? 'afternoon' :
                         lowerMsg.includes('evening') ? 'evening' : 'night';
    return {
  ...
  if (lowerMsg.includes('stressed') || lowerMsg.includes('anxious') || lowerMsg.includes('worried') || lowerMsg.includes('overwhelmed')) {
    return {
      content: "I hear you - feeling stressed can be really tough. While I'm not a therapist, I'm here to listen.\n\n**Some things that might help:**\n• Take a few deep breaths\n• Step away from screens for a bit\n• Write down what's on your mind\n• Talk to someone you trust\n\nIf it's financial stress, I can definitely help break things down into manageable steps. Sometimes the numbers feel less scary when you have a plan.\n\nWhat's weighing on you?",
      confidence: CONFIDENCE.ADVISORY,
      category: RESPONSE_CATEGORIES.CONVERSATION,
      sources: [],
    };
  ...
  if (lowerMsg.includes('happy') || lowerMsg.includes('excited') || lowerMsg.includes('great day') || lowerMsg.includes('good day')) {
    return {
      content: "That's awesome! I love the positive energy! 🌟\n\nIs there anything exciting happening, or are you just feeling good? Either way, it's great to hear. What would you like to chat about?",
      confidence: CONFIDENCE.ADVISORY,
      category: RESPONSE_CATEGORIES.CONVERSATION,
      sources: [],
    };
  ...
  // Goodbye
  if (lowerMsg.includes('bye') || lowerMsg.includes('goodbye') || lowerMsg.includes('see you') || lowerMsg.includes('gotta go')) {
    return {
      content: "Take care! It was great chatting with you. Come back anytime - I'll be here! 👋",
      confidence: CONFIDENCE.ADVISORY,
      category: RESPONSE_CATEGORIES.CONVERSATION,
      sources: [],
    };
        ...
        <div className="message-content">
          ...
          {isAssistant && message.requiresDisclaimer && (
            <div className="message-disclaimer">
              ...
              <span>Investment decisions should be made with a licensed financial advisor.</span>
            </div>
          )}
...
export default function AIChat() {
  const [messages, setMessages] = useState([
    {
      id: 1,
      role: 'assistant',
      content: "Hey there! I'm your ReconAI Assistant. I'm here to help with financial questions, answer your queries, or just have a friendly chat.\n\n**Some things I can help with:**\n• Budgeting and saving tips\n• Tax questions and financial planning\n• General questions and conversation\n• Fun facts and more!\n\nWhat would you like to talk about?",
      timestamp: new Date(),
      confidence: CONFIDENCE.ADVISORY,
      category: RESPONSE_CATEGORIES.CONVERSATION,
      sources: [],
  ...
  const clearChat = () => {
    setMessages([{
      id: Date.now(),
      role: 'assistant',
      content: "Fresh start! I'm ready to chat about anything - finances, questions, or just casual conversation.\n\nWhat's on your mind?",
      timestamp: new Date(),
      confidence: CONFIDENCE.ADVISORY,
      category: RESPONSE_CATEGORIES.CONVERSATION,
      sources: [],
        ...
        <>
          ...
          {showDisclaimer && (
            <div className="advisory-banner">
              ...
              <button className="banner-dismiss" onClick={() => setShowDisclaimer(false)} aria-label="Dismiss">
                <X size={14} />
              </button>
            </div>
          )}
