...
const DISCLAIMER = "Financial data shown is for informational purposes. Please verify critical figures with your accounting team or licensed CPA.";
...
function generateResponse(userMessage) {
  ...
  if (lowerMsg.includes('pull') || lowerMsg.includes('generate') || lowerMsg.includes('show me') || lowerMsg.includes('get me')) {
    if (lowerMsg.includes('report') || lowerMsg.includes('statement') || lowerMsg.includes('summary')) {
      const reportType = lowerMsg.includes('revenue') ? 'Revenue' :
                         lowerMsg.includes('expense') ? 'Expense' :
                         lowerMsg.includes('cash') ? 'Cash Flow' :
                         lowerMsg.includes('p&l') || lowerMsg.includes('profit') ? 'P&L' :
                         lowerMsg.includes('balance') ? 'Balance Sheet' :
                         lowerMsg.includes('board') ? 'Board Summary' : 'Financial';
  ...
  // Revenue questions
  if (lowerMsg.includes('revenue') || lowerMsg.includes('sales') || lowerMsg.includes('income')) {
    return {
  ...
  // Expense questions
  if (lowerMsg.includes('expense') || lowerMsg.includes('cost') || lowerMsg.includes('spending')) {
    return {
  ...
  // Cash flow questions
  if (lowerMsg.includes('cash flow') || lowerMsg.includes('cashflow') || lowerMsg.includes('liquidity')) {
    return {
  ...
  // Forecasting
  if (lowerMsg.includes('forecast') || lowerMsg.includes('predict') || lowerMsg.includes('projection') || lowerMsg.includes('next quarter') || lowerMsg.includes('next year')) {
    return {
  ...
  // Compare / YoY / trends
  if (lowerMsg.includes('compare') || lowerMsg.includes('yoy') || lowerMsg.includes('year over year') || lowerMsg.includes('trend')) {
    return {
  ...
  // Compliance / audit
  if (lowerMsg.includes('compliance') || lowerMsg.includes('audit') || lowerMsg.includes('tax')) {
    return {
  ...
  // Metrics / KPIs
  if (lowerMsg.includes('metric') || lowerMsg.includes('kpi') || lowerMsg.includes('performance')) {
    return {
  ...
  if (lowerMsg.includes('thank') || lowerMsg.includes('thanks')) {
    return {
      content: "You're welcome! I'm here whenever you need financial insights or reports.\n\nIs there anything else I can help you analyze or prepare?",
      confidence: CONFIDENCE.ADVISORY,
      category: RESPONSE_CATEGORIES.GENERAL,
      sources: [],
    };
        ...
        <div className="message-content">
          ...
          {isAssistant && message.requiresDisclaimer && (
            <div className="message-disclaimer">
              ...
              <span>Forecasts are projections based on current data and assumptions.</span>
            </div>
          )}
...
export default function CFOChat() {
  ...
  const clearChat = () => {
    setMessages([{
      id: Date.now(),
      role: 'assistant',
      content: "Chat cleared. I'm ready to help with your financial analysis.\n\nWhat would you like to explore?",
      timestamp: new Date(),
      confidence: CONFIDENCE.ADVISORY,
      category: RESPONSE_CATEGORIES.GENERAL,
      sources: [],
        ...
        <>
          ...
          <div className="chat-footer">
            <span className="refresh-time">
              <Clock size={10} />
              Data synced: {lastRefresh.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
            </span>
          </div>
        </>
      )}
