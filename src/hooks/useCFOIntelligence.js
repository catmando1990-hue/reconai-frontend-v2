...
function normalizeSignal(raw) {
  return {
    ...
    advisory_disclaimer: raw.advisory_disclaimer ?? raw.advisoryDisclaimer ?? null,
...
async function fetchCFOIntelligence() {
  ...
  const [alertsRaw, insightsRaw] = await Promise.all([
    ...
    cfoApi.getInsights().catch(err => {
      console.warn('[useCFOIntelligence] Failed to fetch insights:', err.message);
      return null;
    }),
  ]);
  ...
  const alertSignals = Array.isArray(alertsRaw?.signals) ? alertsRaw.signals
    : Array.isArray(alertsRaw) ? alertsRaw
    : [];
  const insightSignals = Array.isArray(insightsRaw?.signals) ? insightsRaw.signals
  ...
  const lifecycle = (alertsRaw?.lifecycle === 'pending' || insightsRaw?.lifecycle === 'pending')
    ? 'pending'
  ...
  return {
    ...
    reason_message: alertsRaw?.reason_message ?? insightsRaw?.reason_message ?? null,
    signals,
  };
}
...
export function useCFOIntelligence(options = {}) {
    ...
    try {
      ...
      if (response.lifecycle === 'success') {
        ...
        const filteredSignals = includeLowConfidence
          ? allSignals
          : allSignals.filter(s => s.confidence >= DEFAULT_CONFIDENCE_THRESHOLD);
        ...
        setState(prev => ({
          ...
          reasonMessage: response.reason_message || 'CFO Intelligence is processing. Please wait.',
        }));
        ...
        setState(prev => ({
          ...
          allSignals: [],
          isLoading: false,
          error: new Error(response.reason_message || 'Intelligence analysis failed'),
          lifecycle: 'failed',
          reasonCode: response.reason_code,
          reasonMessage: response.reason_message || 'Unable to complete intelligence analysis',
          lastUpdated: null,
        }));
      } else {
  ...
  useEffect(() => {
    if (state.allSignals.length > 0) {
      const filteredSignals = includeLowConfidence
        ? state.allSignals
        : state.allSignals.filter(s => s.confidence >= DEFAULT_CONFIDENCE_THRESHOLD);
