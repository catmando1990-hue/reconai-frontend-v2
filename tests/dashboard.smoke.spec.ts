...
function formatConsoleLocation(loc?: {
  url?: string;
  lineNumber?: number;
  columnNumber?: number;
}) {
  if (!loc || (!loc.url && loc.lineNumber == null && loc.columnNumber == null))
    return "";
...
for (const route of routes) {
  test(`smoke ${route}`, async ({ page }, testInfo) => {
    ...
    if (status >= 500) {
      ...
      expect(
        status,
        `5xx on route ${route} (finalUrl=${finalUrl})`,
      ).toBeLessThan(500);
    }
    ...
    if (!isSignInRedirect) {
      ...
      expect(
        consoleFindings,
        `Console findings on ${route} (finalUrl=${finalUrl})`,
      ).toEqual([]);
    }
  });
}
